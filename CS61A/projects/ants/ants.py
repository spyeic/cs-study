...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        while not temp.bees:
            temp = temp.entrance
            if temp == beehive or count >= self.max_range:
                return None
            count += 1
        print(f"DEBUG: {count}")
...
class ContainerAnt(Ant):
    ...
    def can_contain(self, other):
        # BEGIN Problem 9
        "*** YOUR CODE HERE ***"
        if isinstance(other, Ant) and not isinstance(other, ContainerAnt):
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def action(self, gamestate):
        ...
        if not self.is_real:
            ...
            print(f"DEBUG: {self.armor}, {self.is_real}")
            return
        temp = self.place.exit
        while temp is not None:
            if temp.ant:
                if not temp.ant.doubled:
                    temp.ant.damage *= 2
                    temp.ant.doubled = True
                if isinstance(temp.ant, ContainerAnt) and temp.ant.contained_ant and not temp.ant.contained_ant.doubled:
                    temp.ant.contained_ant.damage *= 2
                    temp.ant.contained_ant.doubled = True
            temp = temp.exit
